 ...
 * @type {Reports}
 */
var reports = new Reports( config, pg, twitter, logger );
		...
		pg.connect(config.pg.conString, function(err, client, done){
			if (err) {
				...
				if (reconnectionAttempts >= config.pg.reconnectionAttempts) {
					...
					reports.tweetAdmin( 'Postgres connection cannot be re-established, shutting down', function() {
						exitWithStatus(1);
					});
				} else {
...
process.on('uncaughtException', function (err) {
	...
	logger.error("Fatal error: Application shutting down");
	reports.tweetAdmin( 'Uncaught exception, shutting down', function() {
		exitWithStatus(1);
	});
});
...
if ( !reports.areTweetMessageLengthsOk() ) {
